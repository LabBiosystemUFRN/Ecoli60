...
if len(sys.argv) != 2:
    print "The number of top genes to be used is necessary"
    exit(1)
nGenes = int(sys.argv[1])
#nGenes = 10
#print("Processing "+str(nGenes)+"...\n")
...
gene_data = parse_file.gene_list()
...
files=["highlyEGLeGac"]
listName=dataDirOut+"listCai"+str(nGenes)+"Genes.csv"

for indexF in range(len(files)):
    filename=dataDirIn+files[indexF]

    #check number of lines available
    numLines = 0
    ...
    if numLines<nGenes:
        nGenes = numLines-1
        print("Expression file have just "+str(nGenes)+" genes...")
    print("Processing top "+str(nGenes)+" genes...\n")
    #exit()
#    inFile.seek(-1,2)     # go to the file end.
#    eof = inFile.tell()   # get the end of file location
#    print(eof)
    inFile.seek(0,0) 
    linha=0
    #inFile = open(filename,"r")
    ...
    countGenes=0
    #for line in inFile:
    while countGenes < nGenes: 
        line = inFile.readline()
        if linha > numLines: #inFile.tell() > eof:
            break
        linha+=1
        ...
        #            codonCount[codons[index]] = codonCount[codons[index]]+1
                    ...
                    count+=1
                countGenes+=1
                listLineOut=geneName+","+geneDict[geneName][4].replace(',','')
                listFile.write("%s\n" % (listLineOut))
                #print countGenes
            else:
...
# write Ws to a file
outFileName = dataDirOut+"cCodonsW"+str(nGenes)+".csv"
outFile = open(outFileName,"w")        
outFile.write("%s\n" % ('amino,ref,w'))
amino = "Y"
...
outFileName = dataDirOut+"cDeltaW"+str(nGenes)+".csv"
outFile = open(outFileName,"w")
#header
outFile.write("%s\n" % ('ref,mut,dw'))
