...
data_directory='../data_files/preProcess/'
...
def annotate_variant2(position, allele, gene_data, position_gene_map):
    ...
    return gene_name, var_type, codon, new_codon, strand, oriented_gene_sequence, gene_start_position, gene_end_position
...
# Reads through the Genbank file for the reference and 
# compiles a list of genes, tRNAs, etc.
# Modified by Clovis Reis to return products too
#    
#####################################################################        
def gene_list(reference_sequence=None, filename="../additional_data/REL606.6.gbk"):
        ...
        if feature=='CDS':
            ...
            while line.split()[0] not in features:
                ...
                if line.startswith('/product'):
                    product = line.split('=')[1].strip()[1:-1] 
    ...
    gene_names, start_positions, end_positions, promoter_start_positions, promoter_end_positions, gene_sequences, strands, products = (list(x) for x in zip(*sorted(zip(gene_names, start_positions, end_positions, promoter_start_positions, promoter_end_positions, gene_sequences, strands, products), key=lambda pair: pair[1])))
    
    return gene_names, numpy.array(start_positions), numpy.array(end_positions), numpy.array(promoter_start_positions), numpy.array(promoter_end_positions), gene_sequences, strands, products
