  ...
  for( col in colnames(mutAllel[7:128])){
    ...
    if(normalize){
      ...
      top100$cNorm<-apply(X = top100[,c("factor","count")],
        ...
        #qtCountTop[is.na(qtCountTop)]<-0
        # qtCountTop$sum<-apply(qtCountTop[,-1],
        #                    MARGIN = 1,
        #                    FUN = sum)
        #qtCountTop<-qtCountTop[qtCountTop$sum !=0,c("ref","sum")]
...
enrichFailZeros<- function(type="HighLow",
                  pval=0.01,
                  quant=7, 
                  normalize = F, 
                  rank=0.2, 
...
plotEnrDeplOld<-function(type="High",
                          pval=0.001,
                          quant=7, 
                          normalize = F, 
                          normBy = "count",
...
timelineOld<-function(file="AuxFiles/bSumHigh.csv",
                   normalize = F,
                   type = "box",
                   save = F,
                   normType = "perK",
